...
import { DynamoEntityIndexesSchema, DynamoEntityIndexSchema } from './../metadata/entity-metadata';
...
export abstract class BaseTransformer {
  ...
  applyClassTransformerFormations<Entity>(
    ...
    const version = schemaVersionAttribute
      ...
      : undefined;
    const transformedPlainEntity = instanceToPlain<Entity>(entity, {
      enableImplicitConversion: true,
      excludePrefixes: ['__'], // exclude internal attributes
      version,
    });

    // convert all Sets back to Set after transformation
    Object.keys(entity as object).forEach(key => {
      if (isSet(entity[key as keyof Entity])) {
        transformedPlainEntity[key] = new Set(transformedPlainEntity[key]);
      }
    });
  ...
  toDynamoEntity<Entity>(entity: Entity) {
    ...
    this.connection.getAttributesForEntity(entityClass).forEach(attr => {
      ...
      if (!entityKeys.includes(attr.name)) {
